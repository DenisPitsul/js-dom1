...
const user = {
  ...
  getBirthdayStr() {
    return `${user.birthday.getFullYear()}-${
...
loadUserBtn.addEventListener("click", (e) => {
    ...
    `
        <article class="card">
            <div class="card-photo-wrapper">
                <img class="card-photo" src="${user.profilePhoto}" alt="${
      user.firstName
    } photo">
            </div>
            <section class="card-main-section">
                ...
                <p class="card-likes"><i class="card-likes-icon fa-solid fa-heart"></i> <span class="card-likes-span">${
                  user.likesCount
                }</span></p>
            </section>
        </article>
  ...
  birthdayEl.onmouseout = (e) =>
    (birthdayEl.textContent = user.getBirthdayStr());
});
